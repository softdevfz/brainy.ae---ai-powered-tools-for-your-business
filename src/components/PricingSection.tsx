      ...
      <div className="container mx-auto px-6">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6 max-w-7xl mx-auto">
          {pricingPlans.map(plan => (
            <div key={plan.productId} className="bg-slate-800 rounded-2xl shadow-2xl p-6 border border-slate-700 flex flex-col">
              <h3 className="text-xl font-bold text-white">{plan.productName}</h3>
              <p className="text-sm text-slate-400 mb-4">{plan.productTagline}</p>
              
              <div className="my-4 text-left">
                <span className="text-4xl font-black text-white">${plan.price}</span>
                <span className="text-lg text-slate-400">/ month</span>
                <p className="mt-2 text-sky-400 text-sm font-semibold">{plan.trial_days}-day free trial</p>
              </div>
